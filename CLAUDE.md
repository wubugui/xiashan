...
## 🐛 项目阶段：已封板，只修 bug，禁止加 feature

**游戏已完成最终开发，进入修 bug 阶段（2026-06 起）。** 此后：

- **禁止添加任何新功能 / 新内容 / 新系统 / 新玩法**——只做缺陷修复、体验修正、数值/文案订正。
- 拿不准某改动算不算「新 feature」时，先问用户，**不要擅自扩功能**。

## 🚀 部署：验证通过后自动 push + 部署（用户已长期授权）

**用户已长期授权**：修完 bug、本地验证（`tsc` / `eslint` / `build` 全绿 + 必要的运行时验证）通过后，**自动 push 到 main 并部署上线，无需逐次询问**。

- 部署动作 = 运行 `zsh tools/deploy-to-server.sh`（构建→精简→压缩→SSH 传到部署服务器并重载 nginx）。服务器：阿里云轻量 `admin@47.116.0.213`，nginx 监听 8080，线上地址 `http://47.116.0.213:8080/`。
- 提交只 stage 本次修复相关文件，**不要**捎带用户在途的 WIP（android/ 打包、资源改名、package.json 等无关漂移）；commit message 末尾保留 Co-Authored-By 行。
- 部署后用「线上 bundle hash == 本地构建 hash」校验确认上线生效。
- 前置硬门槛：本地验证必须全绿才推 / 才部署；验证不过就停下报告，绝不带着红灯上线。
